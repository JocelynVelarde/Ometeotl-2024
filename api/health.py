...
def fetch_pollen_concentration(date: str):
    ...
    api_url = f'https://api.meteomatics.com/{date}--{date}:PT1H/grass_pollen:grainsm3/{lat},{lon}/html?access_token={token}'
    try:
...
def fetch_pollen_warning(date: str):
    ...
    api_url = f'https://api.meteomatics.com/2024-10-06ZP1D:PT1H/grass_pollen_warning:idx/{lat},{lon}/html?access_token={token}'
    try:
        response = requests.get(api_url)
        response.raise_for_status() 
        return response.content.decode("utf-8")
